...
void si_gpu_dump_report(void)
{
	...
	SI_GPU_FOREACH_COMPUTE_UNIT(compute_unit_id)
	{
        ...
        fprintf(f, "L1_TLB.tlb_entry_num = %u\n",
			compute_unit->vector_mem_unit.tlb_entry_num);
        fprintf(f, "L1_TLB.tlb_miss = %u\n",
            compute_unit->vector_mem_unit.tlb_miss);
        fprintf(f, "L1_TLB.tlb_access = %u\n",
            compute_unit->vector_mem_unit.tlb_access);
        fprintf(f, "L1_TLB.tlb_miss_rate = %lf\n",
            (double) compute_unit->vector_mem_unit.tlb_miss / (double)compute_unit->vector_mem_unit.tlb_access);

        fprintf(f, "L1_TLB.vtl_max = %x \n",
			compute_unit->vector_mem_unit.vtl_max);
        fprintf(f, "L1_TLB.vtl_min = %x \n",
            compute_unit->vector_mem_unit.vtl_min);
