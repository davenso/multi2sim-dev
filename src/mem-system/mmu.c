...
struct gpu_mmu_internal_page_t
{
    ...
    unsigned int page_addr[1024];  /* The  page number address saved in this page*/
...
static struct gpu_mmu_internal_page_t *gpu_mmu_get_page_32bit_walk(int address_space_index, unsigned int base_addr, unsigned int page_index){
    struct gpu_mmu_internal_page_t *prev;
    ...
    if(page->page_addr[page_index] == 0x0){
        child_page = xcalloc(1, sizeof(struct gpu_mmu_internal_page_t));
        child_page->address_space_index = address_space_index;
        child_page->base_addr = list_count(gpu_mmu->page_list) << gpu_mmu_log_page_size;
        page->page_addr[page_index] = child_page->base_addr;

        unsigned int addr_index;
        for(addr_index = 0; addr_index < PAGE_ENTRY_NUM; addr_index++){
            child_page -> page_addr[ addr_index ] = 0x0;
        }
        /* Insert in page list */
        list_add(gpu_mmu->page_list, child_page);

        /* Insert in page hash table */
        child_index = (((child_page->base_addr) >> gpu_mmu_log_page_size) + address_space_index * 23) % GPU_MMU_PAGE_HASH_SIZE;
...
unsigned int gpu_mmu_translate_32bit_pagewalk(int address_space_index, unsigned int vtl_addr, unsigned int page_phys_addr ,int mode){
    ...
    assert(page);
    if(mode ==  PDE_MODE){
        phy_addr = page->page_addr[page_index];
    }
    else if(mode ==  PTE_MODE){
        phy_addr = (page->page_addr[page_index] & (~OFFSET_MASK)) | offset;
    }
    else{
        assert(0);
    }
    // yk: 0x0 is used for initial value
    assert(phy_addr != 0x0);
    return phy_addr;
}
...
unsigned int gpu_mmu_get_pde_addr(int address_space_index,unsigned int vtl_addr){
    unsigned int ret_addr;
    struct gpu_mmu_internal_page_t *page;
    int index;
    // yk: Check if the page directory is allocated
    // yk: page base ptr is checked at the first called time
    // yk: Allocate a redundant page at address 0x0
    if(gpu_mmu->page_base_ptr == 0x0){
        page = xcalloc(1, sizeof(struct gpu_mmu_internal_page_t));
        page->base_addr = 0x0;
        page->address_space_index = -1;
        /* Insert empty page in page list */
        list_add(gpu_mmu->page_list, page);
        /* Doesn't insert in hash table */
        ...
        /* Insert in page hash table */
        index = (((page->base_addr) >> gpu_mmu_log_page_size) + address_space_index * 23) % GPU_MMU_PAGE_HASH_SIZE;
        page->next = gpu_mmu->page_hash_table[index];
        gpu_mmu->page_hash_table[index] = page;
    }

    assert(gpu_mmu->page_base_ptr != 0x0);
